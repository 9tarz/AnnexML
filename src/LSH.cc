...
void LSH::SetParams(const size_t K, const double W, const size_t L, const size_t D){
  K_ = K;
  W_ = W;
  L_ = L;
  D_ = D;
...
void LSH::BuildHashTable(const float *data_vec,
                     size_t vec_size,
  ...
  for (size_t n = 0; n < indices_.size(); n++){
    size_t idx = indices_[n];
    ...
    for (size_t l = 0; l < L_; ++l){
      ...
      if (searchKey != hashTable_.end()) {
        hashTable_[hashKey].push_back(idx);
      } else {
        std::vector<size_t> bucket;
        bucket.push_back(idx);
        hashTable_.insert({hashKey, bucket});
      }
    }
  }
...
size_t LSH::Size() const {
  ...
  size_t bucket_count = hashTable_.bucket_count();
  float max_load_factor = hashTable_.max_load_factor();
  if (max_load_factor > 1.0) {
    index_size += (bucket_count * max_load_factor);
  } else {
